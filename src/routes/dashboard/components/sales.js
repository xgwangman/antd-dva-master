...
function Sales ({ data }) {
  return (
    <div className={styles.sales}>
      <div className={styles.title}>年销售额</div>
      <ResponsiveContainer minHeight={360}>
        <LineChart data={data}>
          ...
          <Line type="monotone" dataKey="服装" stroke={color.red} strokeWidth={3} dot={{ fill: color.red }} activeDot={{ r: 5, strokeWidth: 0 }} />
          <Line type="monotone" dataKey="电子" stroke={color.green} strokeWidth={3} dot={{ fill: color.green }} activeDot={{ r: 5, strokeWidth: 0 }} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  )
